...
<head>
                ...
                <div class="pure-u-24-24 pw-30">
                    <form class="pure-form pure-form-stacked" onsubmit="saveRemoteLinkRegions(this);return false;">
                            ...
                            <fieldset>
                                ...
                                <label for="stacked-email">Интервал обновления погоды (в минутах)</label>
                                <input name="intervalTimeRefresh" id="interval-time-refresh" class="w-100" type="text" placeholder="в минутах" />
                            </fieldset>
                            <button type="submit" class="pure-button pure-button-primary w-100 pure-button-middle">Сохранить</button>
                    </form>
    ...
    <script>
                ...
                fetch('/remote-server/load')
                    ...
                    .then((data) => {
                        ...
                        if (data != null) {
                            ...
                            document.getElementById('interval-time-refresh').value = data.intervalTimeRefresh;
                        }
                    })
                    .catch(() => {
                        document.getElementById('meteostation').style.opacity = '1';
        ...
        function saveRemoteServer(e) {
            let params = new URLSearchParams(new FormData(e)).toString();
            document.getElementById('meteostation').style.opacity = '0.5';
            fetch(`/remote-server/save?${params}`)
                .then((response) => response.json())
                .then((data) => {
                    document.getElementById('meteostation').style.opacity = '1';
                    if (data != null) {
                        alert('Параметры настроек метеостанции сохранены!');
                        return;
                    }

                    alert('Ошибка при сохранение параметров настроек метеостанции!');
                })
                .catch(() => {
                    document.getElementById('meteostation').style.opacity = '1';
                    alert('Ошибка при сохранение параметров настроек метеостанции!');
